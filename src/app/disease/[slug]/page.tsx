...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  let titleText = disease.title;
  if (typeof titleText === 'object' && titleText !== null) {
      titleText = titleText['@value'] || "";
  }
  const title = titleText ? String(titleText).replace(/<[^>]*>/g, '') : "Disease Detail";
...
export default async function DiseasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Helper to safely get definition and title
  const definition = disease.definition ? disease.definition['@value'] || disease.definition : "No definition provided by WHO ICD.";
  
  // Title can also be an object with @value property
  let titleEncoded = disease.title;
  if (typeof titleEncoded === 'object' && titleEncoded !== null) {
    titleEncoded = titleEncoded['@value'] || "";
  }
  const titleVisible = titleEncoded ? String(titleEncoded) : "Unknown Condition";
  const titleSlug = titleVisible.replace(/<[^>]*>/g, '');
        ...
        <div className="container px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            <div>
              ...
              <h1 className="text-3xl md:text-5xl font-bold font-serif text-[#111111] mb-4" dangerouslySetInnerHTML={{ __html: titleVisible }}>
              </h1>
