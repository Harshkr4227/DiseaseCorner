    ...
    <div className="container px-4 md:px-6 py-12">
      ...
      {results.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {results.map((disease) => {
             // Extract ID from URI (last part of the URL)
             const entityId = String(disease.uri).replace(/\/$/, '').split('/').pop() || disease.id;
             return (
            <Link href={`/disease/${entityId}`} key={disease.id} className="block h-full group">
              <Card className="h-full border-2 border-[#111111] shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all bg-white rounded-md">
                <CardHeader className="border-b border-[#111111]/5 pb-4">
                  <div className="flex justify-between items-start">
                    <CardTitle 
                      className="text-xl text-[#111111] font-serif font-bold group-hover:underline decoration-1 underline-offset-4 line-clamp-2"
                ...
                <CardContent className="pt-4">
                  <CardDescription 
                    className="line-clamp-3 text-[#333333] leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: disease.definition || "No definition available." }}
                  />
                </CardContent>
